 ...
 *  ]);
 *
 *  // $config 中, 'maxSize', 'allowFiles', 'pathFormat' 一般为必须项. 其他项在上传非图片及不需要缩略图时, 不需要配置
 */
class Uploader
{
    ...
    {
        // 获取上传图片的宽高值
        if(!($imgInfo = self::getImageInfo($tempName))){
            return false;
        }
