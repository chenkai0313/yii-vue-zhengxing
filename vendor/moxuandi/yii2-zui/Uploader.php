...
class Uploader
{
    ...
    public function __construct($config)
    {
        ...
        if($_POST['chunks'] > 1){
            ...
            if($this->file = UploadedFile::getInstanceByName('file')){
    ...
    private function checkUpload($file)
    {
        ...
        $this->realName = isset($_POST['name']) ? $_POST['name'] : $file->name;
    ...
    public function saveDatabase()
    {
        ...
        $model->file_mime = mime_content_type($this->fullName);
     ...
     */
    //分片暂存区
    private $chunkpath = 'uploads/chunk';
    ...
    {
        if(!self::createDir($this->chunkpath))
            return false;

        //移动分片
        move_uploaded_file($_FILES['file']['tmp_name'], $this->chunkpath . '/chunk.tmp_' . $_POST['chunk']);

        //判断是否是最后一片，如果是则进行文件合成并且删除文件片
        if(($_POST['chunk'] + 1) == $_POST['chunks']){
            $this->realName = $_POST['name'];
            $this->fileExt = Helper::getExt($this->realName);
            $this->fullName = self::getFullName();
            ...
            for($i=0; $i < $_POST['chunks']; $i++){
                $blob .= file_get_contents($this->chunkpath . '/chunk.tmp_' . $i);
            }
            file_put_contents($this->fullName, $blob);

            //删除分片缓存
            for($i=0; $i < $_POST['chunks']; $i++){
                unlink($this->chunkpath . '/chunk.tmp_' . $i);
            }
            //删除分片暂存区
            rmdir($this->chunkpath);

            $this->fileMime = mime_content_type($this->fullName);
