...
class UEditorAction extends Action
{
	...
	public function init()
	{
		...
		$_config = require(__DIR__ . '/config.php');	// 默认设置
		$this->config = ArrayHelper::merge($_config, $this->config);	// 默认设置
		parent::init();
	}
	...
	public function run()
	{
        switch(yii::$app->request->get('action')){
            case 'config': $result = json_encode($this->config); break;
                /* 上传图片 */
            case 'uploadimage':
                /* 上传涂鸦 */
            case 'uploadscrawl':
            ...
            default: $result = json_encode(['state'=>'请求地址出错']); break;
        }
        /* 输出结果 */
	...
	protected function actionList()
	{
		...
		/* 返回数据 */
		return json_encode(['state'=>'SUCCESS', 'list'=>$list, 'start'=>$start, 'total'=>count($files)]);
	}
			...
			if($file != '.' && $file != '..'){
				...
				if(is_dir($path2)){
					...
					if(preg_match("/\.(" . $allowFiles . ")$/i", $file)){
						$files[] = ['url'=>substr($path2, strlen($_SERVER['DOCUMENT_ROOT'])), 'mtime'=>filemtime($path2)];
					}
				}
			}
		}
